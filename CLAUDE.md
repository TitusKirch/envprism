...
Distributed as a CLI (`npx envprism`, `pnpm dlx envprism`). See `TEMP_AI.md` for the original bootstrap brief (delete once the project is past bootstrap). The detailed v0 feature plan lives at `/root/.claude/plans/purring-wishing-tiger.md`.

## Stack

Mirrors `../forgemap` (the kirchDev CLI house style):

- **Node 24+** (pinned via `.nvmrc`) and **pnpm 11** (`packageManager` + `.npmrc`), ESM only (`"type": "module"`).
- **TypeScript strict** (`noUncheckedIndexedAccess`, `verbatimModuleSyntax`, `allowImportingTsExtensions` with `noEmit`).
- **Vite (lib mode)** bundles to `dist/bin/envprism.mjs` (with shebang banner) and `dist/index.mjs`. Runtime deps are externalised in `vite.config.ts` — update `runtimeDeps` there when adding a new runtime dep, or the bundle will inline it.
- **Vitest** (config inlined in `vite.config.ts` via `vitest/config`'s `defineConfig`).
- **citty** for CLI parsing, **consola** for non-TUI output, **pathe** for paths.
- **opentui** is the planned TUI framework (primary candidate; `ink`/`blessed` as fallback). Not yet installed — added in the TUI PR.
